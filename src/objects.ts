...
export function isCorrect(question: Question, answer: string): boolean {
    const y = question.expected.toLowerCase();
    const z = answer.toLocaleLowerCase();
    const value = z.trim() === y.trim() ? true : false;
    return value;
}
...
export function isValid(question: Question, answer: string): boolean {
    let value = question.type === "short_answer_question" ? true : false;
    if (!value) {
        const final = question.options.filter(
            (option: string): boolean => option === answer
        );
        value = final.length > 0 ? true : false;
    }
    return value;
}
...
export function toShortForm(question: Question): string {
    const word = question.id + ": " + question.name.slice(0, 10);
    return word;
}
...
export function toMarkdown(question: Question): string {
    let solution = "# " + question.name + "\n" + question.body;
    if (question.type === "multiple_choice_question") {
        const x = question.options.map(
